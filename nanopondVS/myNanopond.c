...
/* SDL refresh frequency  smaller value = more refreshes*/
...
enum COLORSCHEME { KINSHIP, LINEAGE, MAX_COLOR_SCHEME };
enum COLORSCHEME colorScheme = KINSHIP;
...
struct PerReportStatCounters
{
	...
	* executed since the last report. */
	uint64_t instructionExecutions[16];

	/* Number of cells executed since last report */
	uint64_t cellExecutions;
...
static void doReport(const uint64_t clock)
{
	...
	printf(\
		"Clock: %" PRIu64 "\n" /* clock */				\
		"totalEnergy: %" PRIu64 "\n" /* totalEnergy */			\
		"totalActiveCells: %" PRIu64 "\n" /* totalActiveCells */			\
		"totalViableReplicators: %" PRIu64 "\n" /* totalViableReplicators */		\
		"maxGeneration: %" PRIu64 "\n" /* maxGeneration */			\
		"viableCellsReplaced: %" PRIu64 "\n" /* statCounters.viableCellsReplaced */	\
		"viableCellsKilled: %" PRIu64 "\n" /* statCounters.viableCellsKilled */	\
		"viableCellShares: %" PRIu64 "\n",/* statCounters.viableCellShares */	\
		clock,
		totalEnergy,
		totalActiveCells,
		totalViableReplicators,
	...
	* instruction per cell execution. */
	...
	for (x = 0; x<16; ++x) {
		...
		printf("Instruction %d executions: %.4e\n", x, ((statCounters.cellExecutions > 0.0) ? (statCounters.instructionExecutions[x] / statCounters.cellExecutions) : 0.0));
	}

	/* The last column is the average metabolism per cell execution */
	printf("Average metabolism: %.4e\n", (statCounters.cellExecutions > 0.0) ? (totalMetabolism / statCounters.cellExecutions) : 0.0);
	fflush(stdout);
...
/* Utility for checking if valid cell chosen in thread*/
int radius = 3;
int cellChosen[NUM_THREADS][2];
int validCell(int x, int y) {
	int i;
	for (i = 0; i < NUM_THREADS; i++) {
		if (cellChosen[i][0] != -1) {
			if ((abs(x - cellChosen[i][0]) < radius) && (abs(y - cellChosen[i][1]) < radius)) {
				return 0;
			}
		}
	}
...
#pragma omp parallel private(x, y, i, cell, tmcell, tn, ptr_wordPtr, ptr_shiftPtr, reg, tmp, loopStackPtr, wordPtr, shiftPtr, facing, falseLoopDepth, stop, currentWord, inst) \
			...
			* clockNP ticks. */
			if (!(clockNP % INFLOW_FREQUENCY) && tn == 1) {
				...
				cell->energy += INFLOW_RATE_BASE + (getRandom(tn) % INFLOW_RATE_VARIATION);
#else
				cell->energy += INFLOW_RATE_BASE;
#endif /* INFLOW_RATE_VARIATION */
				for (i = 0; i<POND_DEPTH_SYSWORDS; ++i) {
					cell->genome[i] = getRandom(tn);
				}
#pragma omp critical
				{
					++cellIdCounter;
...
//			}
			...
			while (cell->energy && (!stop)) {
				...
				* ranges of the genome, etc. */
				if ((getRandom(tn) & 0xffffffff) < MUTATION_RATE) {
					tmp = getRandom(tn); /* Call getRandom() only once for speed */
